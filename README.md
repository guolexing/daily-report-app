...
- ⏰ **Windows 原生提醒**：任务开始时间系统 Toast 通知（无需浏览器），已通知记录持久化，重启不重复提醒
- 🖥️ **关闭窗口后台运行**：点 X 关闭窗口后应用驻留系统托盘，任务提醒照常生效；点击托盘图标即可重新打开，右键托盘可退出
- 🤖 **AI 智能汇总**：接入 OpenAI 兼容接口，一键生成周报/月报摘要
- 📋 **周报栏目列表（自定义栏目 + 提示词）**：周报结构由「栏目列表」定义，可自由增删栏目（本周进展 / 本周未完成 / 下周工作计划 / 风险与问题…）
- ✍️ **每栏目独立撰写要求**：每个栏目可单独填写写作要求（提示词），AI 严格按各栏目要求生成；留空则用默认规则
- 🪄 **示例解析生成要求**：不想写要求？粘贴一段该栏目「期望的样子」，AI 自动解析出该栏目的撰写要求
- 🔗 **任务管理联动**：AI 生成周报时自动读取任务数据——「本周未完成」取未完成任务（含计划日期），「下周工作计划」由下周计划任务 + 逾期需顺延任务 + 日报后续计划合成
- 🧠 **AI 智能导入**：粘贴任意格式的日程文本，AI 自动识别并结构化导入
- 💾 **MySQL 持久化**：数据存本地 MySQL，打开 APP 自动读写；支持全库备份导出/恢复（.json），数据可移植到任意电脑
...
数据可移植：APP 内「数据管理 → MySQL 数据存储 → ⬇ 导出全库备份」生成 .json 全库备份文件；拷贝到任意电脑安装本 APP 后「⬆ 从备份恢复」即可完整还原（恢复前自动备份当前库到本机 `backups/` 目录）。
